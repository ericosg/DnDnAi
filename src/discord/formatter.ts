...
import type { DiceResult, CombatState, Player, GameState } from "../state/types.js";
...
export function combatStatusEmbed(combat: CombatState, players: Player[]): EmbedBuilder {
...
export function inventoryEmbed(player: Player): EmbedBuilder {
  ...
  const items = cs.equipment.length
    ? cs.equipment.map((e) => `• ${e}`).join("\n")
    : "No equipment";
