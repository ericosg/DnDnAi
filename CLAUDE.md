...
- **Campaign Blueprint**: `dm-notes/campaign.md` is a structured plot skeleton always injected into the DM prompt. Contains acts with checkboxed milestones, boss encounters (unique combat mechanics, lairs, stakes), escalation triggers (referencing the long rest counter), world consequences, and resolution conditions. Generated by the DM on `/start` (new games) or `/resume` (retrofit). The DM can adapt *how* milestones are reached but cannot delete uncompleted milestones or boss encounters. A "World Clock" header shows `gameState.longRestCount` so escalation triggers create urgency — villains advance their plans on long rests whether or not the party engages.
