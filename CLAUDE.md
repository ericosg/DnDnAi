...
bun test                       # Run unit tests (967 tests)
...
|------|-------|--------|--------|
| DM | Opus | default (high on guardrail retry) | Narrative quality, rule adjudication |
| Agents | Sonnet | low (medium on guardrail retry) | Good roleplay, cost-effective (per-agent override via frontmatter) |
| Orchestrator | Haiku | default | Fast flow control, mostly deterministic |
| Guardrail | Haiku | default | Reviews DM output for player agency violations and agent output for world-fact invention |
...
- **Agentic Agents**: AI agents (`ai/agent.ts`) also run agentic, scoped to `--allowedTools Read,Edit`. Each agent has a first-person memory file at `data/games/<id>/agent-notes/<slug>.md`. The absolute path is in the agent's system prompt; the current contents are embedded in the user message each turn. Agents Edit the file to append bullets under `## What I Remember`, `## What I Carry`, `## What I Know About Myself`, `## Bonds & Relationships`, or `## Open Threads` after meaningful events. They cannot Write (so they can't create arbitrary files) and have no Glob/Grep (no codebase snooping). The system prompt also includes explicit "Information Boundaries" forbidding reads of `src/`, `docs/`, `dm-notes/`, other characters' files, and game state JSON — trust model matches the DM but with a single allowed file path.
- **Agent player-parity**: Agents have the same abilities human players do, exposed as directives in their output: `[[PASS]]` (equivalent to `/pass`), `[[ASK:question]]` (`/ask`), `[[LOOK:target]]` (`/look`), and `[[WHISPER:Name TEXT:msg]]` (`/whisper`). Parsed by the pure function `processAgentDirectives()` in `game/agent-directives.ts`; side effects (calling the DM, posting answers, DMing whisper targets) are executed in `engine.ts:applyAgentAction()` after the agent's IC response posts. `/character`, `/inventory`, `/status`, and `/recap` don't need directives — the agent's prompt includes full character sheet (via `buildCharacterReference` from `dm-prompt.ts`), party HP/conditions, narrative summary, and scene state on every turn.
- **Agent Memory (persistent)**: Agent memory files live in `data/games/<id>/agent-notes/<slug>.md`, structured as first-person markdown with five fixed sections. Seeded from the character sheet + personality on `/add-agent` join (or on `[[ACTIVATE:]]` for dormant agents). For in-flight games that predate this module, `/resume` detects a missing `agent-notes/` directory and runs a one-time retrofit: a Sonnet call per active AI agent distills memory from full history + `dm-notes/characters/<name>.md` + the character sheet (see `ai/agent-memory-seed.ts`). The DM can force-commit entries mid-turn via `[[REMEMBER:name TEXT:...]]` — useful for correcting mechanical mistakes (e.g., Nyx's non-existent Light cantrip) and locking in key beats. Side effects live in `game/agent-memory-effects.ts` and fire after every `processDirectives()` call.
- **DM Notes (persistent memory)**: The DM maintains notes in `data/games/<id>/dm-notes/` — running context (`dm.md`, always loaded), campaign blueprint (`campaign.md`, always loaded), character details learned in RP (`characters/{name}.md`), world state (`world.md`), plot threads (`plot.md`), rules rulings (`rulings.md`), and a session log (`session-log.md`). The DM reads these before responding and writes to them when it learns something new. Directory is seeded with templates on `/start`. These files serve a different purpose from `agent-notes/` — DM notes are out-of-character (mechanics tracking, corrections log); agent notes are in-character (what the character remembers and feels).
...
- **Directive processing**: All directive parsing and application is extracted into `game/directives.ts` as a pure function `processDirectives()`. This takes DM response text and game state, processes all directives (ROLL, DAMAGE, HEAL, UPDATE_HP, UPDATE_CONDITION, REQUEST_ROLL, SPELL, USE, CONCENTRATE, CONDITION, XP, INVENTORY, GOLD, REST, COMBAT signals, ACTIVATE, REMEMBER), mutates game state, and returns a `DirectiveContext` with the processed text and metadata. This enables testing directives independently from the engine's I/O concerns. I/O side effects for agent memory (seeding on activate, appending on remember) are applied by `game/agent-memory-effects.ts` after every `processDirectives()` call.
- **Canonical facts injection**: The `⚠️ CANONICAL FACTS` section from `dm-notes/world.md` is read at prompt-build time and injected directly into the DM system prompt (Layer 1c). This prevents hallucination of names, locations, and established facts — the DM can't skip reading its notes because the facts are already in its prompt. Backed by a MANDATORY pre-narration NPC check rule in `DM_IDENTITY` (verify NPC gender/pronouns/status against canon before naming) AND a `## ⚠️ FINAL REMINDER — RE-READ CANONICAL FACTS BEFORE NARRATING` block appended after all dynamic prompt content (combat, pendingRolls, etc.) — uses recency to keep canon attended to even after long sliding-history slices. The same facts are fed into narrative compression with an explicit "appear verbatim" instruction to prevent paraphrase or omission. DM is also instructed to emit `[[REMEMBER:...]]` when correcting mechanical mistakes (non-existent spells, misremembered features) so the correction persists in the agent's memory file.
- **Campaign Blueprint**: `dm-notes/campaign.md` is a structured plot skeleton always injected into the DM prompt. Contains acts with checkboxed milestones (each with XP rewards), boss encounters (unique combat mechanics, lairs, stakes), side quests (optional content with hooks, objectives, and rewards), escalation triggers (referencing the long rest counter), world consequences, and resolution conditions. Generated by the DM on `/start` (new games) or `/resume` (retrofit). The DM awards XP via `[[XP:...]]` directives when milestones are completed. The DM can adapt *how* milestones are reached but cannot delete uncompleted milestones, boss encounters, or side quests. A "World Clock" header shows `gameState.longRestCount` so escalation triggers create urgency — villains advance their plans on long rests whether or not the party engages.
- **Narration quality guardrail**: `isToolMetaOnly()` in `game/engine.ts` detects when the DM responds with only tool meta-commentary (e.g., "Updated dm-notes.") instead of actual narration. Triggers a retry with explicit feedback at `effort: high`. Prevents the narration debt bug where players see tool operation messages instead of story. Requires BOTH a narrative marker (`*`, `>`, `[[`) AND `length >= 150` to bypass the meta-verb check — a stray decorative marker around tool-meta prose is not enough.
- **Admin OOC corrections (`/correct`)**: The bot owner (auth via `ADMIN_USER_ID` env var) can issue OOC corrections to the DM, an agent, all, or a specific human via `/correct target:<dm|all|agent-name> message:<text> remember:<bool> user:<User?>`. The in-memory store in `game/admin-corrections.ts` (mirrors `ask-history.ts`) holds up to 10 entries per game, auto-expiring after 5 turns. Corrections are pinned at the TOP of the next prompt under `## ⚠️ Admin Correction (READ FIRST — overrides anything below)` — high-salience injection that overrides everything else. When `remember:true` and target is an agent, the message is also auto-appended to that agent's memory file via `appendAgentMemory`. Corrections are also persisted as system `TurnEntry` records for durable audit. `/clear-corrections` empties the in-memory buffer. The agent guardrail still enforces world-fact rules — admin-injected facts agents try to declare get blocked and naturally refactor into IC questions, which the DM ratifies on the next turn (self-healing layered guardrail behavior).
- **Hard facts in agent memory**: Bullets prefixed with `- !! ` (in any of the 5 memory sections) are extracted by `extractHardFacts()` in `game/agent-notes.ts` and pinned at the END of the agent's system prompt under `## CRITICAL — Hard Facts From Your Memory`. Uses prompt recency to defend against long-context drift. Authored manually or by the admin via `/correct ... remember:true` (with `!! ` in the message body for the prefix to carry through). Documented in `docs/creating-agents.md`.
- **Agent memory audit log**: `game/agent-memory-audit.ts` writes a one-line audit record to `data/games/<id>/agent-notes/.audit/<slug>.log` whenever an agent uses `Edit` or `Write` against its own memory file via `chatAgentic`. Purely observational — no enforcement, no guardrail. The audit dir is gitignored (parent `data/games/<id>/` is). Wired via a new `onToolUse?: (event: ToolUseEvent) => void | Promise<void>` callback parameter on `chatAgentic` in `ai/claude.ts`.
- **Stale `pendingRolls` cleanup**: `autoResolveStalePendingRolls()` runs at the start of every `orchestratorLoop` iteration. For `pendingRolls` owed by an AI agent that are older than 10 minutes (`STALE_PENDING_ROLL_MS`), the engine auto-rolls the dice and appends a `system` history entry. Prevents deadlocks when the DM mistakenly issues `REQUEST_ROLL` for an agent (which should auto-resolve but somehow didn't). Humans always have to roll themselves — never auto-resolved.
- **No-promise retry on `/ask`**: `containsPromisePattern()` in `ai/dm-prompt.ts` matches `I'll (track|fix|handle|note|remember|update|check|work|make|add|put|record|log)` in DM `/ask` answers (supports both straight and curly apostrophes). On match, `dmAsk` retries once with explicit feedback telling the DM to act now rather than promise. Punts get re-rolled into action.
